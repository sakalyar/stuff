 ...
 *             forall i in [0..|gSS| - 2] : gSS[i] >= MIN_FRAGMENT_SIZE
 *             gSS[|gSS| - 1] >= 1
 *             sum(i in [0..|gSS| - 1], gSS[i]) == f.length()
 *             getSplitsNames() != null
 *             |getSplitsNames()| == |gSS|
 *             forall i in [0..|getSplitsNames()| - 1] :
 *                 getSplitsNames()[i]
...
public interface SplitManager {
     ...
     *         lg  ::= getFile().length()
     *     |gSS| == ceil(lg / size)
     *     forall i in [0..|gSS| - 2] : gSS[i] == size
     *     gSS[|gSS| - 1] == lg - sum(i in [0..|gSS| - 2], gSS[i]) </pre>
     */
     ...
     *     sizes != null
     *     1 <= |sizes|
     *     sum(i in [0..|sizes| - 1], sizes[i]) < getFile().length()
     *         ==> |sizes| < getMaxFragmentNb()
     *     sum(i in [0..|sizes| - 1], sizes[i]) >= getFile().length()
     *         ==> |sizes| <= getMaxFragmentNb()
     *     forall i in [0..|sizes| - 1] : sizes[i] >= MIN_FRAGMENT_SIZE </pre>
     * @post <pre>
     *     Let gSS ::= getSplitsSizes()
     *         lg  ::= getFile().length()
     *     lg <= sum(i in [0..|sizes| - 1], sizes[i]) ==> |gSS| <= |sizes|
     *     lg > sum(i in [0..|sizes| - 1], sizes[i]) ==> |gSS| == |sizes| + 1
     *     forall i in [0..|gSS| - 2] : gSS[i] == sizes[i]
     *     gSS[|gSS| - 1] == lg - sum(i in [0..|gSS| - 2], sizes[i]) </pre>
     */
     ...
     *     |gSS| == splitsNb
     *     q >= MIN_FRAGMENT_SIZE
     *         ==> forall i in [0..r - 1] : gSS[i] == q + 1
     *             forall i in [r..|gSS| - 1] : gSS[i] == q
     *     q < MIN_FRAGMENT_SIZE
     *         ==> forall i in [0..|gSS| - 2] : gSS[i] == MIN_FRAGMENT_SIZE
     *             gSS[|gSS| - 1] == n - sum(i in [0..|gSS| - 2], gSS[i]) </pre>
     */
     ...
     *     canSplit() </pre>
     * @post <pre>
     *     forall i in [0..|getSplitsSizes()| - 1] :
